...
energyOffer(ca,dt,o,blk,'price') $ { (co2cost(dt,o) > 0) and (energyOffer(ca,dt,o,blk,'price') > 1) }
        = max[1, energyOffer(ca,dt,o,blk,'price') - co2cost(dt,o) ]; 
...
In our second scenario, we assume that these low-priced energy offers (<$1/MWh) from 
hydro generators would remain as currently offered as these offers do not appear 
to include the ETS cost effects. The higher-priced offers are adjusted to account for 
ETS cost effects as discussed in Section 2.5 below
...
energyOffer(ca,dt,hydro_offers(o),blk,'price') $ (energyOffer(ca,dt,o,blk,'price') > 1)
        = max[1, energyOffer(ca,dt,o,blk,'price') - Average_CO2_cost(ca,dt) ]; 
